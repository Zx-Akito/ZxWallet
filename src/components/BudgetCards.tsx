    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-zinc-900/60 border border-zinc-800 p-3.5 sm:p-4.5 rounded-2xl">
        <div className="flex items-center space-x-2.5 sm:space-x-3">
          ...
          <div>
            <h3 className="text-xs sm:text-sm font-semibold text-zinc-100">Batas Anggaran Bulanan</h3>
            <p className="text-[11px] sm:text-xs text-zinc-400">Atur kuota belanja per kategori agar tidak boros</p>
          </div>
        </div>
      ...
      {budgets.length === 0 ? (
        <div className="py-12 text-center text-zinc-500 text-xs bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6">
          Belum ada batas anggaran. Klik tombol &quot;Tambah Anggaran&quot; di atas untuk membatasi pengeluaran bulanan.
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="font-semibold text-sm text-zinc-200 truncate">{b.category}</span>
                ...
                <div className="flex justify-between items-center text-xs text-zinc-400 font-mono">
                  <span>Terpakai: <b className="text-zinc-200">{formatIDR(b.spent)}</b></span>
                  <span>Batas: <b className="text-zinc-200">{formatIDR(b.monthly_limit)}</b></span>
                </div>

                <div className="text-xs text-zinc-400 mt-1.5 text-right font-mono">
                  Sisa Kuota: <span className={`font-semibold ${b.remaining === 0 ? 'text-rose-400' : 'text-emerald-400'}`}>{formatIDR(b.remaining)}</span> ({b.percentage}%)
                </div>
              </div>
            );
          })}
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-sm w-full p-5 shadow-2xl animate-pop-in">
            <h3 className="text-base font-semibold text-zinc-100 mb-1">Tambah Batas Anggaran</h3>
            <p className="text-xs text-zinc-400 mb-4">Pilih kategori pos belanja dan kuota bulanan maksimal.</p>

            <form onSubmit={handleSave} className="space-y-3.5">
              <div>
                <label className="block text-xs text-zinc-300 mb-1 font-medium">Kategori Pengeluaran / Pos Belanja</label>
                <CustomDropdown
                  value={selectedCat}
              ...
              <div>
                <label className="block text-xs text-zinc-300 mb-1 font-medium">Batas Maksimal Bulanan (Rp)</label>
                <input
                  type="number"
                  required
